...
a = Analysis(['count.py'],
             ...
             binaries=[
               ...
               # FFMPEG reqs
               (osp.join(conda_base, 'ffmpeg.exe'), '.'),
               (osp.join(conda_base, 'avfilter-7.dll'), '.'),
               (osp.join(conda_base, 'avcodec-58.dll'), '.'),
               (osp.join(conda_base, 'avformat-58.dll'), '.'),
               (osp.join(conda_base, 'avdevice-58.dll'), '.'),
               (osp.join(conda_base, 'avutil-56.dll'), '.'),
               (osp.join(conda_base, 'postproc-55.dll'), '.'),
               (osp.join(conda_base, 'swresample-3.dll'), '.'),
               (osp.join(conda_base, 'swscale-5.dll'), '.'),
               ],
             datas=[
               *collect_data_files("utils.utils", include_py_files=True),
               ('count.spec', 'Library/bin'), # Just to force create this directory
               ],
             hiddenimports=[],
             hookspath=[],
             hooksconfig={},
